...
case \$1 in
 start) diskmond 
        ;;
  stop) killall diskmond
        ;;
     *) echo "diskmond [start|stop]"
        ;;
esac
EOF
...
cat >> "${BUILDDIR}/etc/pkg_stx/config" <<EOF
# This is a blank configuration file for pkg_stx
# See /usr/share/pkg_stx.txt for information 
EOF
